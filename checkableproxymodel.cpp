...
CheckableProxyModel::CheckableProxyModel(QObject *parent) :
    ...
    // I have adjusted the numbers below from 30,000 and 1,000. When the numbers are set this high,
    // this model has trouble picking up all of the files on rapid, consectutive operations. -CRV
    m_cleanupTimer(new DelayedExecutionTimer(3000, 100, this)),
...
{
    /* The purpose of the below "if" statement is that if the model has already been set, then nothing
     * needs to be reset and all of the user's checks can remain if this function is called again. */
...
Qt::CheckState CheckableProxyModel::resolveCheckStateRole(QModelIndex sourceIndex) const
{
    ...
    // Shuts up compiler about control reaching end of non-void function. The switch above covers all cases.
    return Qt::PartiallyChecked;
}
...
void CheckableProxyModel::removeSubtree(QModelIndex sourceIndex)
{
    ...
    for (int i(0); i<rowCount; ++i) {
        ...
        //Old: QPersistentModelIndex pIndex(sourceIndex.child(i, 0));
        if (m_checkStates.contains(pIndex)) {
            removeSubtree(sourceIndex.model()->index(i, 0, sourceIndex));
            //Old: removeSubtree(sourceIndex.child(i, 0));
        }
    ...
    if (rowCount > 0) {
        ...
        //Old: emit dataChanged(sourceIndex.child(0,0), sourceIndex.child(rowCount-1,0));
    }
}
